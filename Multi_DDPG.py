...
from src.agent import DDPGAgent, LinearAgent, Mcpc
...
def train(args):
    ...
    mcpc_agent             = Mcpc(camp_info=camp_info,
                                max_bid_price=args['ddpg_max_bid_price'],
                                budget=train_budget)
    ...
    for step in range(int(train_iteration)):
        ...
        episode.append([agent0_action, agent1_action, linear_action, mcpc_action, bid['pctr'], bid['click']])
        ...
        if reward[0] == 1:     # ddpg0 win
            ...
            if agent1_action != 0.:
                ddpg_agent1.update_result(is_win = False)
            linear_action.update_result(is_win=False)
